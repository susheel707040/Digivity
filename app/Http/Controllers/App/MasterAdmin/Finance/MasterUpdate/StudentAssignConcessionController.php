...
class StudentAssignConcessionController extends Controller
{
        ...
        */
        ...
        if ($request->hasFile('import_file')) {
            ...
            foreach ($importdata as $key => $data) {
                if ($key != 0) {
                    $importdataarr[] = ['admission_no' => $data[0], 'fee_concession' => $data[1]];
                }
            }
        ...
        return view('app.erpmodule.MasterAdmin.Finance.MasterUpdate.student-assign-concession', compact(['studentgroup','importdata']));
    }
    ...
    public function store(Request $request)
    {
        ...
        if (count($request->student_id)) {
            ...
            foreach ($request->student_id as $studentid) {
                ...
                $student->update(['fee_concession_id' => $request["student_" . $studentid . "_concession_type_id"]]);
            }
